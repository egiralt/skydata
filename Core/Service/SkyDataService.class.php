...
class SkyDataService extends SkyDataResource implements IService
{
	...
	private $InjectedFunctions;
	private $InjectedVariables;

	public function InjectFunction ($scriptFileName, $namedParametersArray, $loadOnStart = false)
    {
        // Si el nombre del fichero es una ruta relativa, se le agrega la ruta actual
        if (substr($scriptFileName,0,1) !== '/')
            $scriptFileName = realpath(ReflectionFactory::GetClassDirectory (get_class($this)).'/'.$scriptFileName);
            
        if (is_file($scriptFileName))
        {
            $scriptInfo = new \stdClass();
            $scriptInfo->parameters = $namedParametersArray;
            $scriptInfo->script = $scriptFileName;
            $scriptInfo->loadOnStart = $loadOnStart;            

            $this->InjectedFunctions[$scriptFileName] = $scriptInfo;
        }
    }

    public function InjectVariable ($variableName, $initValue)
    {
        $variableInfo = new \stdClass();
        $variableInfo->variable = $variableName;
        $variableInfo->value = $initValue;

        $this->InjectedVariables[] = $variableInfo;
    }
	...
	public function RenderServiceJavascript()
	{
        ...
        // Leer el contenido de los scripts inyectables 
        $InjectedFunctions = array();
        foreach ($this->InjectedFunctions as $fileName => $info)
        {
            
            $newInfo = clone $info;
            $newInfo->script = file_get_contents(realpath($fileName));
            $newInfo->name = basename($fileName, '.js');
            $InjectedFunctions[] = $newInfo;
        }
        
		// Preparar los parámetros para la plantilla
		//echo "<pre>";print_r ($dataModel); die();
		$params = array (
		    'base_path'          => $this->GetApplication()->GetApplicationBaseUrl(),
			'serviceName'        => $this->GetClassShortName(),
			'tables'             => $dataModel,
			'methods' 		     => $otherMethods,
			'is_global'          => $this->IsGlobal(),
			'InjectedFunctions'    => $InjectedFunctions,
            'injectedVariables'  => $this->InjectedVariables
		);
	...
	{
		$result = array();
        //echo "<pre>"; print_r ($this->GetController ()); die();
