 ...
 * 
 * 
 * por: Ernesto Giralt Hernández...27/Marzo/2014
 * 
 * 
 */
namespace SkyData\Libraries\ElasticSearch;

DEFINE ('INDEX_NAME', 'vgrs');
DEFINE ('ES_DEFAULT_PORT', 9200);
DEFINE ('ES_SERVER', 'localhost');
DEFINE ('MAIN_SEARCH_TYPE', 'vehiculos');
DEFINE ('MAIN_URL', sprintf ('%s/%s/_search', ES_SERVER, INDEX_NAME));
 ...
 */
 class ElasticSearchManager
 {
	 ...
	 * 
	 */
	public function DoCompletionSuggest ($needle, $field, $routing = null)
	{
		$result = array();
		
		$query->suggest_query = new \stdClass();
		$query->suggest_query->text = $needle;
	...
	public function GetItemByID ($type, $id, $routing = null)
	{
		$result = null;
	...
	public function DoQuery($query, $type = null, $url = null, $routing = null, $method = "GET")
	{
		...
		curl_setopt($ch, CURLOPT_PORT, ES_DEFAULT_PORT);	
	...
	{
		return sprintf ('%s:%s/%s', ES_SERVER, ES_DEFAULT_PORT, $this->GetIndexName());		
	}	 	
...
/*	 
	 private static function _getListaConcesionariosByDNI ($dni)
	 {
	 	$result = array();
		$db = DBManagerFactory::getInstance();
		$set = $db->query ("SELECT id_concesionario FROM crm.gesco_concesionarios_clientes_auth WHERE dni = '$dni';");
		while ($row = $db->fetchByAssoc($set))
			$result[] = $row['dni'];
 ...
 */
	 ...
	 /**
	  * Retorna un elemento por su UUID
	  */
	 public function GetItem ($type, $uuid, $routing = null, $field_id = 'uuid')
	 {
	 	$result = null;
	 	$resultSet = $this->GetItemsByFieldValue($type, $field_id, $uuid, $routing, 1);
	 ...
	 //TODO: Refactoring: sustituir 'GetItemsByFieldsValue' por este método 
	 public function GetItemsByFieldsValue ($type, $pairsKV, $routing = null, $count = null)
	 {
	 	$customURL = $this->BuildSearchQueryURL($type, false, $routing);
	 	
	 	$query->query->bool->must = array();
	 	foreach ($pairsKV as $key => $value )
		{
			$match = new \stdClass();
	  ...
	  */
	 public function GetItemsByFieldValue ($type, $fieldName, $value, $routing = null, $count = null)
	 {
	 	$customURL = $this->BuildSearchQueryURL($type, false, $routing);
	 	$query->query->match->$fieldName = ($value === null ? '' : $value);// Hay que garantizar que no de error por un null mal puesto
		if ($count !==  null && is_numeric($count))
			$query->size = $count;
	  ...
	  */
	 public function GetItemByFieldsValue ($type, $pairKV, $routing = null)
	 {
	 	$result = null;
	 	$resultSet = $this->GetItemsByFieldsValue($type, $pairKV, $routing, 1);
	  ...
	  * Retorna el primer elemento que cumpla con el valor en el campo específicado
	  */
	 public function GetItemByFieldValue ($type, $fieldName, $value, $routing = null)
	 {
	 	$result = null;
	 	$resultSet = $this->GetItemsByFieldValue($type, $fieldName, $value, $routing, 1);
	...
	{
		$scriptFileName = realpath(dirname(__FILE__).'/scripts/'.$scriptName.'.json');
		if (!file_exists($scriptFileName))
			throw new Exception(sprintf ("No existe el script solicitado: %s [%s]", $scriptFileName, $scriptName), 1000); 
